...
typedef struct
{
    const char *ossl_name; /* The OpenSSL cipher name */
...
static PRInt32 layer_recv(PRFileDesc *fd, void *buf, PRInt32 amount,
                            PRIntn flags, PRIntervalTime timeout)
{
  const PRRecvFN recv_fn = fd->lower->methods->recv;
  PRInt32 rv;

  while ((rv = recv_fn(fd->lower, buf, amount, flags, timeout)) < 0)
  {
    Vio *vio= SSL_get_ex_data((SSL *)fd, 0);
...
static PRInt32 layer_send(PRFileDesc *fd, const void *buf, PRInt32 amount,
                          PRIntn flags, PRIntervalTime timeout)
{
  const PRSendFN send_fn = fd->lower->methods->send;
  PRInt32 rv;

  while ((rv= send_fn(fd->lower, buf, amount, flags, timeout)) < 0)
  {
    Vio *vio= SSL_get_ex_data((SSL *)fd, 0);
    if (rv != PR_WOULD_BLOCK_ERROR)
      break;
...
int SSL_library_init(void)
{
    ...
    gIdentity = PR_GetUniqueIdentity("MariaDB");
    if (gIdentity == PR_INVALID_IO_LAYER) {
      fprintf(stderr, "Failed to init NSS stub layer\n");
      return 1;
    }
    ...
    /* overwrite some methods by our internal wrappers */
    gMethods.close = layer_close;
/*    gMethods.recv= layer_recv;
    gMethods.send= layer_send; */
                           ...
                           PRBool server)
{
    ...
    if (layer == NULL)
        goto error;
    if (PR_PushIOLayer(s, PR_GetLayersIdentity(s), layer) != PR_SUCCESS)
        goto error;
    ...
    if (SSL_OptionSet(s, SSL_REQUEST_CERTIFICATE, PR_TRUE) != SECSuccess)
        goto error;

    if (SSL_OptionSet(s, SSL_REQUIRE_CERTIFICATE, SSL_REQUIRE_NEVER) != SECSuccess)
        goto error;

    if (SSL_OptionSet(s, SSL_HANDSHAKE_AS_SERVER, PR_TRUE) != SECSuccess)
        goto error;

    if (SSL_OptionSet(s, SSL_ENABLE_TLS, PR_TRUE) != SECSuccess)
        goto error;
...
{
    return create_context(PR_TRUE, PR_TRUE, PR_FALSE, PR_FALSE);
}
...
static const char *ma_nss_ossl_cipher_name(const PRUint16 cipher)
{
  unsigned int i;
  for (i=0; i < MAX_NSS_CIPHERS; i++)
  {
    if (tls_ciphers[i].sid == cipher)
...
int na_nss_get_cipher_list(SSL *ssl, char *buff, size_t length)
{
  int i;
  char *p= buff;
  const PRUint16  cipher_cnt= SSL_GetNumImplementedCiphers();
  ...
  *p= 0;
  ...
  for (i=0; (i < cipher_cnt) && (p - buff < length); i++)
  {
    ...
    if (SSL_CipherPrefGet(ssl, ciphers[i], &enabled) != SECSuccess)
      return 0;
    if (enabled) {
      const char *cipher= ma_nss_ossl_cipher_name(ciphers[i]);
      if (cipher && (p + strlen(cipher) < buff + length - 1))
      {
        if (p > buff) {
          strcat(p, ":");
          p++;
...
int SSL_CTX_check_cert_attributes(X509 *cert, char *attribute, enum x509_attr_type type)
{
  char *p= NULL, *token= NULL;

  switch (type) {
  case ATTR_ISSUER:
    p= cert->cert->issuerName;
    break;
  case ATTR_SUBJECT:
    p= cert->cert->subjectName;
    break;
  default:
    return 0;
  }
...
const char *SSL_CIPHER_map_name(SSL_CIPHER *c)
{
  SSLChannelInfo      channel;
  SSLCipherSuiteInfo  suite;
      ...
      channel.cipherSuite)
  {
    ...
    for (i=0; i < MAX_NSS_CIPHERS; i++)
    {
      if (tls_ciphers[i].sid == channel.cipherSuite)
        return tls_ciphers[i].name;
    }
        ...
        if (SSL_GetCipherSuiteInfo(channel.cipherSuite,
            ...
            switch (channel.protocolVersion) {
                ...
                case SSL_LIBRARY_VERSION_TLS_1_1:
                    protocol = PORT_ArenaStrdup(ossl->arena, "TLSv1.1");
                    break;
                case SSL_LIBRARY_VERSION_TLS_1_2:
                    protocol = PORT_ArenaStrdup(ossl->arena, "TLSv1.2");
                    break;
#ifdef SSL_LIBARY_VERSION_TLS_1_3
                case SSL_LIBRARY_VERSION_TLS_1_3:
                    protocol = PORT_ArenaStrdup(ossl->arena, "TLSv1.3");
                    break;
#endif
            }
        }
...
{
  return PR_FileDesc2NativeHandle(s);
}
...
int SSL_accept(SSL *ssl)
{
    ...
    {
      fprintf(stderr, "Handshake failed: %s (%d)\n",  PR_ErrorToName(PR_GetError()), PR_GetError());
      goto error;
    }
